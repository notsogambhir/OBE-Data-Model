"""
CO Attainment Calculator — Beginner Friendly Version
=====================================================
Calculates how well students achieved Course Outcomes (COs).

Install: pip install pandas openpyxl
Run:     python GEM_fixed.py
"""
...
def calculate_co_attainment(input_file):
    ...
    # Open the Excel file (we'll read multiple sheets from it)
    excel = pd.ExcelFile(input_file)

    # ─────────────────────────────────────────────
    # STEP 1: Read settings from OBE Details sheet
    # ─────────────────────────────────────────────
    # The first sheet has the target %, weights, and level thresholds.

    obe = excel.parse(0, header=None)  # Read first sheet without headers

    # Defaults (used if we can't find values in the sheet)
    target = 60.0
    weights = {}
    levels = []

    # Scan every row to find settings
    reading = None  # Keeps track of what section we're in
    ...
    levels.sort(reverse=True)  # Check highest level first
    ...
    # ─────────────────────────────────────────────
    # We look for sheet pairs like "ST1 Ques Mapping" + "ST1 Result"
    ...
    for sheet in excel.sheet_names:
        ...
        # Get exam name: "ST1 Ques Mapping" → "ST1"
        name = sheet.replace(' Ques Mapping', '').replace(' Mapping', '').strip()
        if name in found:
            continue

        # Find the matching Result sheet
        result_sheet = None
        ...
        if result_sheet:
            # Decide category based on exam name
    ...
    # ─────────────────────────────────────────────
    # STEP 3: For each exam, calculate CO% per student
    # ─────────────────────────────────────────────

    all_students = {}     # roll_no → student_name
    all_cos = set()       # set of all CO names (CO1, CO2, etc.)
    exam_scores = []      # list of {exam_name, category, scores_df}

    for exam in exams:

        # --- Read mapping (which questions test which COs) ---
        mapping = excel.parse(exam['mapping'])
        mapping.columns = [str(c).strip() for c in mapping.columns]

        # Find CO columns (CO1, CO2, CO3, etc.)
        ...
        # Make CO names uppercase for consistency
        for c in co_columns:
            mapping.rename(columns={c: c.upper()}, inplace=True)
        co_columns = [c.upper() for c in co_columns]

        # Get question IDs and max marks
        q_col = mapping.columns[0]   # First column = question ID
        mm_col = mapping.columns[1]  # Second column = max marks

        mapping[q_col] = mapping[q_col].astype(str).str.strip()
        mapping[mm_col] = pd.to_numeric(mapping[mm_col], errors='coerce').fillna(0)

        # Fix boolean values (Excel sometimes stores True/False instead of 1/0)
        ...
        # Remove questions with 0 max marks or no CO mapping
        mapping = mapping[mapping[mm_col] > 0]
        mapping = mapping[mapping[co_columns].sum(axis=1) > 0]

        if mapping.empty:
            print(f"    Skipping {exam['name']}: empty mapping")
            continue
        ...
        # --- Calculate CO% for each student ---
        scores = pd.DataFrame()
        scores['Roll'] = results[roll_col].values

        for co in co_columns:
            # Which questions are linked to this CO?
            ...
            for _, student in results.iterrows():
                ...
                for q in linked_questions:
                    ...
                    # Check if unattempted (U, blank, AB, NaN)
                ...
                if maximum > 0:
                    co_percentages.append((obtained / maximum) * 100)
                else:
                    co_percentages.append(np.nan)
    ...
    # ─────────────────────────────────────────────
    # Example: Final CO1% = (Avg of ST1,ST2 CO1%) × 0.4 + (ETE CO1%) × 0.6
    ...
    for co in all_cos:
        ...
        for category, weight in weights.items():
            ...
            # Get CO% from each exam in this category
            ...
            # Average them, then multiply by weight
            if not pcts.empty:
                avg = pcts.mean(axis=1, skipna=True)
                weighted_total += avg.fillna(0) * weight

        # Save final CO% and whether target was met
    ...
    for co in all_cos:
        ...
        if total_students > 0:
            success_rate = students_passed / total_students
        else:
            success_rate = 0
    ...
    # Print summary
    print(f"\n  {'CO':<6} {'Tried':>6} {'Passed':>7} {'Rate':>8} {'Level':>6}")
    print(f"  {'-'*38}")
    for _, r in summary_df.iterrows():
        print(f"  {r['CO']:<6} {r['Students Attempted']:>6} {r['Met Target']:>7} "
              f"{r['Success Rate %']:>7.1f}% {r['Attainment Level']:>5}")
...
# ─────────────────────────────────────────────
# RUN: Process all Input*.xlsx files
# ─────────────────────────────────────────────

for file in sorted(glob.glob("Input*.xlsx")):
    calculate_co_attainment(file)
